    ...
    public partial class Invoice : Form
    {
        ...
        private void LoadInvoices()
        {
            ...
            if (dgvInvoices.Rows.Count > 0)
            {
                ...
                selectedInvoiceId = (int)dgvInvoices.Rows[0].Tag;
        ...
        private void LoadLineItems(int invoiceId)
        {
            ...
            foreach (var item in items)
            {
                ...
                dgvLineItems.Rows[row].Cells["colDescription"].Value = string.IsNullOrWhiteSpace(item.FeeName) ? $"Fee Rule {item.FeeId}" : item.FeeName;
