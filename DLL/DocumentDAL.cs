...
namespace visavault_g43.DLL
{
    internal class DocumentDAL
    {
        ...
        {
            string query =
                "INSERT INTO document (client_id, type_id, document_no, expiry_date, issue_date) " +
        ...
        public static int UpdateDocument(Document document)
        {
            string query =
                "UPDATE document " +
                "SET client_id = @ClientId, type_id = @TypeId, document_no = @DocumentNo, " +
        ...
        {
            string query =
                "SELECT COUNT(*) FROM renewalcase rc " +
                "JOIN renewalstage rs ON rc.currentstage_id = rs.stage_id " +
                "WHERE rc.document_id = @DocumentId " +
                "AND rs.stage_name != 'Completed' AND rs.stage_name != 'Rejected';";
        ...
        public static DataTable GetCriticalDocuments()
        {
            string query =
                ...
                "JOIN documenttype dt ON d.type_id = dt.documenttype_id " +
                "JOIN client c ON c.client_id = d.client_id " +
                "WHERE DATEDIFF(d.expiry_date, CURDATE()) <= 45 " +
